...
from util import ExtractSubstructureContextPair, PairwiseNodeEdgeMask, ContrastMask1

from dataloader import DataLoaderSubstructContext, DataLoaderMasking, DataLoaderContrast, DataLoaderContrast3
...
def train(args, model, model_dis, model_pred, loader, optimizer, optimizer_dis, optimizer_pred, device, criterion, epoch):
    ...
    for step, batch in enumerate(tqdm(loader, desc="Iteration")):
        ...
        rep_a = model(batch.x_a, batch.edge_index_a, batch.edge_attr_a)
        rep_b = model(batch.x_b, batch.edge_index_b, batch.edge_attr_b)
        # print(rep_a.size())
        # print(batch.batch_a.size())
        ...
        # print(rep_a_global.size())

        # print(batch.masked_atom_indices_a)
        pred_node = model_pred(rep_a[batch.masked_atom_indices_a])
        # print(pred_node.size(), torch.min(batch.mask_node_label_a[:, 0]), torch.max(batch.mask_node_label_a[:, 0]))
        loss = F.cross_entropy(pred_node.float(), batch.mask_node_label_a[:, 0].long())
        # acc_node = compute_accuracy(pred_node, batch.mask_node_label_a[:, 0], batch.masked_atom_indices_a.size(0))
        # acc_node_accum += acc_node

        pred_node = model_pred(rep_b[batch.masked_atom_indices_b])
        # print(pred_node.size(), torch.min(batch.mask_node_label_b[:, 0]), torch.max(batch.mask_node_label_b[:, 0]))
        loss = loss + F.cross_entropy(pred_node.float(), batch.mask_node_label_b[:, 0].long())
        # acc_node = compute_accuracy(pred_node, batch.mask_node_label_b[:, 0], batch.masked_atom_indices_b.size(0))
        # acc_node_accum += acc_node
...
def main():
    ...
    dataset = ContrastDataset("dataset/" + args.dataset, args=args, dataset=args.dataset,
                              transform=ContrastMask1(num_atom_type = 119, num_edge_type = 5, mask_node=True, mask_rate = args.mask_rate, mask_edge=args.mask_edge))
    loader = DataLoaderContrast3(dataset, batch_size=args.batch_size, shuffle=True, num_workers=args.num_workers)
