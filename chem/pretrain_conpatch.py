...
from util import ExtractSubstructureContextPair, PairwiseNodeEdgeMask, ContrastMask1, ContrastMask2

from dataloader import DataLoaderSubstructContext, DataLoaderMasking, DataLoaderContrast, DataLoaderContrast2
...
def train(args, model, model_dis, loader, optimizer, optimizer_dis, device, criterion, epoch, model_dis_ip=None, optimizer_dis_ip=None):
    # model_k.eval()
    # model_q.train()
    model.train()
    model_dis.train()
    if model_dis_ip != None:
        model_dis_ip.train()
    kk = args.kk
    ...
    for j in range(2, kk):
        ...
        offsets.append(art)
    mmb = torch.LongTensor([1 if ii < kk - 1 else 1-kk for ii in range(kk)]).to(device)
    ...
    aaa = []
    for i in range(bsa):
        aaa.append(torch.ones(kk).long().to(device) * i)
    aaa = torch.cat(aaa, dim=0)
    aab = []
    for i in range(bsb):
        aab.append(torch.ones(kk).long().to(device) * i)
    aab = torch.cat(aab, dim=0)
    print(N, bsa, bsb)
    ...
    for step, batch in enumerate(tqdm(loader, desc="Iteration")):
        ...
        rep_subgraph = from_rep_to_subgraph_rep(batch, rep)
        k = batch.k
        bs = rep_global.size(0)

        num_subgraph = rep_subgraph.size(0)


        if args.dis_type_ip == 1:
            rep_subgraph = rep_subgraph / torch.clamp(torch.norm(rep_subgraph, p=2, dim=1, keepdim=True), min=args.eps)
            # repel and attract between subgraphs
            assert num_subgraph % k == 0
            logits = torch.matmul(rep_subgraph, rep_subgraph.t())
            arr = torch.arange(num_subgraph).to(device)
            logits[arr, arr] = -args.inf
            for j in range(2, k):
                # mm = torch.LongTensor([j if (ii + j) < k else j - k for ii in range(k)]).to(device)
                mm = offsets[j - 2]
                mm = mm.repeat(bs)
                logits[arr, arr + mm] = -args.inf
            # mm = torch.LongTensor([1 if ii < k - 1 else -4 for ii in range(k)]).to(device)
            # mm = mm.repeat(bs)
            mm = mmb.repeat(bs)
            target = arr + mm
            logits = logits / T
            ...
            logits = model_dis_ip(rep_subgraph)
            arr = torch.arange(num_subgraph).to(device)
            target = torch.zeros_like(logits)
            target[arr, arr] = 1.
            for j in range(2, k):
                mm = offsets[j - 2].repeat(bs)
                target[arr, arr + mm] = 1.
            loss = criterion_dis(logits.view(-1), target.view(-1))


        # aa = []
        # for i in range(bs):
        #     aa.append(torch.ones(k).long().to(device) * i)
        ...
        if args.dis_type == 1:
            positive_expanded_summary_emb = summary_emb[aa]
            shifted_summary_emb = summary_emb[cycle_index(len(summary_emb), 1)]
            negative_expanded_summary_emb = shifted_summary_emb[aa]
            positive_score = model_dis(rep_subgraph, positive_expanded_summary_emb)
            negative_score = model_dis(rep_subgraph, negative_expanded_summary_emb)

            loss = loss + criterion_dis(positive_score, torch.ones_like(positive_score)) + criterion_dis(negative_score, torch.zeros_like(negative_score))
        elif args.dis_type == 2:
            logits, target = model_dis(rep_subgraph, summary_emb, aa)
            loss = loss + criterion_dis(logits, target)

        # rep_a = model(batch.x_a, batch.edge_index_a, batch.edge_attr_a, batch.num_nodes_a, batch.num_edges_a, batch.filter_k_a)
        # rep_b = model(batch.x_b, batch.edge_index_b, batch.edge_attr_b, batch.num_nodes_b, batch.num_edges_b, batch.filter_k_b)
        # rep_a_global = pool_func(rep_a, batch.batch_a, args.pooling)
        # rep_b_global = pool_func(rep_b, batch.batch_b, args.pooling)
        #
...
def main():
    ...
    args.output_model_file = "temp/contrast_patch_{:d}_dis_type_{:d}_ip_{:d}".format(args.kk, args.dis_type, args.dis_type_ip)

    # set up dataset and transform function.
    dataset = ContrastDataset("dataset/" + args.dataset, args=args, dataset=args.dataset,
                              transform=ContrastMask2(num_atom_type = 119, num_edge_type = 5, mask_rate = args.mask_rate, mask_edge=args.mask_edge, k=args.kk))
    print(len(dataset))
    loader = DataLoaderContrast2(dataset, batch_size=args.batch_size, shuffle=True, num_workers=args.num_workers)
    ...
    #                     gnn_type=args.gnn_type).to(device)
    model_dis = Discriminator(args.emb_dim if args.JK != "concat" else args.emb_dim * (1 + 1), dis_type=args.dis_type).to(device)
    if args.dis_type_ip == 2:
