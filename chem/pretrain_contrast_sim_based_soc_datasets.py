...
from loader import MoleculeDataset, ContrastDataset, MoleculeDatasetForContrastOth
...
def main():
    ...
    args.output_model_file = "temp/contrast_sim_based_vallina_shuffle_true_soc_set_soft_other_sim_other_dataset_{:d}_gnn_{}".format(args.num_samples,
                                                                                                args.gnn_type)
    if args.env == "jizhi":
        args.output_model_file = "/apdcephfs/private_meowliu/ft_local/gnn_pretraining/temp_model/" + \
                                 args.output_model_file.split("/", 1)[1]

    # set up dataset and transform function.
    dataset = MoleculeDatasetForContrastOth("dataset/" + args.dataset, dataset=args.dataset,
                              transform=None, num_samples=args.num_samples, extract_sim_type="sim")
    loader = DataLoaderContrastSimBased(dataset, batch_size=args.batch_size, shuffle=True, num_workers=args.num_workers)
