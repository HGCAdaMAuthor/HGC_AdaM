...
def train(args, model_k, model_q, loader, optimizer_q, device, epoch):
    ...
    for step, batch in enumerate(tqdm(loader, desc="Iteration")):
        ...
        neg_rep_global_all_list = list()
        # pos_rep_global_all_list = list()
        q_rep_global_all_list = list()
...
def main():
    ...
    args.output_model_file = "temp/contrast_sim_based_2_neg_{}_{:d}_num_neg_samples_{:d}_T_{:d}".format(args.neg_sample_stra,
        args.num_samples, args.num_neg_samples, args.T)
    ...
    dataset = MoleculeDatasetForContrast("dataset/" + args.dataset, dataset=args.dataset,
                            transform=None, extract_sim=True, num_samples=args.num_samples, k=args.num_neg_samples,
                                         extract_sim_type=args.neg_sample_stra, with_neg=True, T=args.T)
    loader = DataLoaderContrastSimBased(dataset, batch_size=args.batch_size, shuffle=False,
