...
def main():
    ...
    parser.add_argument('--introduce_random', action="store_true", default = False, help='number of workers for dataset loading')
    parser.add_argument('--mask_stra', type=str, default = "random", help='number of workers for dataset loading')
    args = parser.parse_args()

    args.output_model_file = "temp/attri_masking_introduce_random_{}".format(str(args.introduce_random))
    torch.manual_seed(0)
    np.random.seed(0)
    ...
    dataset = MoleculeDataset("dataset/" + args.dataset, dataset=args.dataset, transform = MaskAtom(num_atom_type = 119,
                                                                                                    num_edge_type = 5, mask_rate = args.mask_rate, mask_edge=args.mask_edge, introduce_random=args.introduce_random, mask_stra=args.mask_stra))
