...
def train(args, model_k, model_q, model_dis, model_attn, loader, optimizer_q, optimizer_dis, optimizer_attn, device,
    ...
    for step, batch in enumerate(tqdm(loader, desc="Iteration")):
        ...
        if args.triplet_manner == True:

            rep = model_q(batch.x, batch.edge_index, batch.edge_attr)
            rep_pos = model_q(batch.x_pos, batch.edge_index_pos, batch.edge_attr_pos)
            rep_neg = model_q(batch.x_neg, batch.edge_index_neg, batch.edge_attr_neg)
            # print(rep.size(), rep_pos.size(), rep_neg.size())

            rep_global = pool_func(rep, batch.batch, args.pooling)
            rep_global_pos = pool_func(rep_pos, batch.batch_pos, args.pooling)
            rep_global_neg = pool_func(rep_neg, batch.batch_neg, args.pooling)
            # print(rep_global.size(), rep_global_pos.size(), rep_global_neg.size())

            bs = rep_global.size(0)
            assert rep_global.size(0) == rep_global_pos.size(0) and rep_global.size(0) == rep_global_neg.size(0)

            positive_score = model_dis(rep_global, rep_global_pos)
            negative_score = model_dis(rep_global, rep_global_neg)
            ...
            rep_global_norm = rep_global / (torch.clamp(torch.norm(rep_global, p=2, dim=1, keepdim=True), min=1e-12))
            logits = torch.matmul(rep_global_norm, rep_global_norm.t()) / T
            ...
            logits = torch.softmax(logits, dim=-1)  # the posibility of each graph similar to another --- or the similarity score...

            positive_samples_idx = torch.multinomial(logits, num_samples=1)
            # print("multinomial sample's shape = ", positive_samples_idx.size())
            if len(positive_samples_idx.size()) == 2:
                positive_samples_idx = positive_samples_idx[:, 0]

            positive_global_rep = rep_global[positive_samples_idx, :]

            if args.reverse_possi == False:
                if args.sublinear == None:
                    negative_samples_idx = torch.multinomial(logits, num_samples=1)
                else :
                    logits = torch.pow(logits, args.sublinear)
                    logits = logits / torch.sum(logits, dim=-1, keepdim=True)
                    negative_samples_idx = torch.multinomial(logits, num_samples=1)
            else:
                negative_logits = torch.softmax(-logits, dim=-1)
                negative_samples_idx = torch.multinomial(negative_logits, num_samples=1)

            if len(negative_samples_idx.size()) == 2:
                negative_samples_idx = negative_samples_idx[:, 0]
            negative_global_rep = rep_global[negative_samples_idx, :]

            positive_score = model_dis(rep_global, positive_global_rep)
            negative_score = model_dis(rep_global, negative_global_rep)
...
def main():
    ...
    if args.triplet_manner == True:
        if args.batch_size % 3 != 0:
            args.batch_size = (args.batch_size // 3) * 3

    torch.manual_seed(0)
    np.random.seed(0)

    if args.triplet_manner == True:
        args.output_model_file = "temp/contrast_triplet_True"
    else:
        args.output_model_file = "temp/contrast_top1_triplet_False_reverse_possi_{}_sublinear_{}".format(str(args.reverse_possi), str(args.sublinear))
    ...
    if args.triplet_manner == False:
        ...
        args.dataset = "chembl_filtered"
        dataset = SplitLabelMoleculeDataset("dataset/" + args.dataset, dataset=args.dataset, transform=None)
        loader = DataLoaderTriplet(dataset, batch_size=args.batch_size, shuffle=False, num_workers=args.num_workers)
