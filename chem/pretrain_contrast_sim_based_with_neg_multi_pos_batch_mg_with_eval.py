...
def get_val_dataloader(args):
    ...
    train_loader = DataLoaderContrastList(train_dataset, batch_size=args.eval_batch_size, shuffle=True, num_workers=args.num_workers)
    # torch_geometric.data.DataListLoader(train_dataset, batch_size=8, shuffle=True)
    val_loader = DataLoaderContrastList(valid_dataset, batch_size=args.eval_batch_size, shuffle=False, num_workers=args.num_workers)
    test_loader = DataLoaderContrastList(test_dataset, batch_size=args.eval_batch_size, shuffle=False, num_workers=args.num_workers)
...
def train_val(args, model, train_loader, optimizer):
    ...
    for step, batch in enumerate(tqdm(train_loader, desc="Iteration")):
        ...
        node_rep = model_gnn(batch.data_list)
        pred = graph_pred_linear(pool_func(node_rep, batch.batch.cuda(), args.graph_pooling))
        # pred = model(batch.data_list)
        y = batch.y.view(pred.shape).to(torch.float64).cuda()
...
def eval_val(args, model, loader):
    ...
    for step, batch in enumerate(tqdm(loader, desc="Iteration")):
        ...
        y_true.append(batch.y.cuda().view(pred.shape))
...
def start_val(dataloaders, args, epoch):
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        # print("val: %f test: %f" % (val_acc, test_acc)) 好的好的 多谢老师~~ 这道题分值实在太高了[泪流满面]
...
def main():
    ...
    model_q = GNN(args.num_layer, args.emb_dim, JK=args.JK, drop_ratio=args.dropout_ratio,
                  gnn_type=args.gnn_type) #.to(device)
    print("got model_q and going to put it into dataparallel model")
    model_q = torch_geometric.nn.DataParallel(model_q.cuda())
    print("after data parallel model built.")
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if epoch % 1 == 0:
            ...
            print('time: %s, epoch = %d, auc = %.4f' % (
            datetime.datetime.now(), epoch, test_acc))
