...
# model=$1
gpu=$1
for epoch in 13 14 15 16
do
epochNum=${epoch}
python finetune.py --input_model_file temp/contrast_sim_based_vallina_rwr_with_neg_1_num_neg_samples_32_${epochNum}.pth --device ${gpu} --test_ratio 0.1 --train_ratio 0.8 --dataset hiv
done
...
# 写脚本做实验 for rwr_neg; for uniform_neg --- 先看一下最大的epoch的结果 --- still not so good 如果可以再看其他epochs的结果; batch_neg :不同epochs的结果？
# 不同的T对于rwr_neg的影响 ---- 等待资源中。。 --- restart prob？