    ...
    for epoch in range(1, args.eval_epochs + 1):
        ...
        if val_acc > best_val_acc:
            ...
            torch.save(model.state_dict(), os.path.join(save_eval_model_folder,
                                                           "contrast_hie_gra_lr_{:.4f}_rw_hops_{:d}_num_path_{:d}_gnn_{}.pth".format(
                                                               args.lr, args.rw_hops, args.num_path, args.val_dataset)))
    model_test = load_test_model(args, os.path.join(save_eval_model_folder,
                                                           "contrast_hie_gra_lr_{:.4f}_rw_hops_{:d}_num_path_{:d}_gnn_{}.pth".format(
                                                               args.lr, args.rw_hops, args.num_path, args.val_dataset)))
    best_test_acc = eval_val(args, model_test, test_loader)
