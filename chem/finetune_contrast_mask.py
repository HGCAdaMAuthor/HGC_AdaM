...
from util import MaskAtomKeptOrigin
import os
import shutil
...
criterion_consistency = nn.MSELoss(reduction="mean")
...
def train(args, model, device, loader, optimizer):
    ...
    for step, batch in enumerate(tqdm(loader, desc="Iteration")):
        ...
        pred_masked = model(batch.masked_x, batch.edge_index, batch.edge_attr, batch.batch)

        pred = model(batch.x, batch.edge_index, batch.edge_attr, batch.batch)
        y = batch.y.view(pred.shape).to(torch.float64)

        pred_logits = F.softmax(pred, dim=-1)
        pred_logits_masked = F.softmax(pred_masked, dim=-1)
        loss_mat_con = criterion_consistency(pred_logits, pred_logits_masked)
        ...
        optimizer.zero_grad()
        loss = torch.sum(loss_mat) / torch.sum(is_valid) + loss_mat_con
        loss.backward()
...
def main():
    ...
    print("best_acc", best_test_acc, best_epoch)
    if not args.filename == "":
        writer.close()
    ...
    if not args.input_model_file == "":
        ...
        with open(os.path.join("./record", ssl), "a") as wf:
            wf.write("dataset: {}, train_ratio: {:.4f}, best epoch: {:d}, best acc: {:.4f}.\n".format(args.dataset,
                                                                                                      args.train_ratio,
                                                                                                      best_epoch,
                                                                                                      best_test_acc))
            wf.close()
