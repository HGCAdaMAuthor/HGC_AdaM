import argparse
# todo masking with really drop out; --- with infomax to be added
from loader import MoleculeDataset
from dataloader import DataLoaderContrast7  # , DataListLoader
...
# context to center node prediction
def train(args, model_list, loader, optimizer_list, device):
    ...
    for step, batch in enumerate(tqdm(loader, desc="Iteration")):
        ...
        rep_context_nodes = node_rep[batch.context_nodes]
        rep_pooled_pred_rep = pool_func(rep_context_nodes, batch.context_nodes_to_idx, args.pooling)
        pred_node = linear_pred_atoms(rep_pooled_pred_rep)
...
def main():
    ...
    args.output_model_file = "temp/denoising_drop_masked_nodes_infomax_{}".format(str(args.infomax))
    print("num layer: %d mask rate: %f mask edge: %d" % (args.num_layer, args.mask_rate, args.mask_edge))

    # set up dataset and transform function.
    dataset = MoleculeDataset("dataset/" + args.dataset, dataset=args.dataset,
                              transform=MaskAtom3(num_atom_type=119, num_edge_type=5, mask_rate=args.mask_rate,
                                                 mask_edge=args.mask_edge))

    loader = DataLoaderContrast7(dataset, batch_size=args.batch_size, shuffle=True, num_workers=args.num_workers)
