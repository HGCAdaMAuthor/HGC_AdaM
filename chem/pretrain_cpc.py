...
from util import  ContrastMask5

from dataloader import DataLoaderContrast5
...
def train(args, model, model_dis, loader, optimizer, optimizer_dis, device, criterion, epoch, model_dis_ip, optimizer_dis_ip):
    ...
    for step, batch in enumerate(tqdm(loader, desc="Iteration")):
        ...
        if args.use_infomax == True:
            summary_emb = torch.sigmoid(rep_global)
...
def main():
    ...
    parser.add_argument("--contrast_ingraph", default=False, action="store_true", help="epsilon, help with the numerical stability.")
    ...
    args.output_model_file = "temp/con_context_cpc_k_{:d}_step_{:d}_infomax_{}_gnn_{}".format(args.kk, args.step, str(args.use_infomax), args.gnn_type)

    # set up dataset and transform function.
    dataset = ContrastDataset("dataset/" + args.dataset, args=args, dataset=args.dataset,
                              transform=ContrastMask5(num_atom_type = 119, num_edge_type = 5, mask_rate = args.mask_rate, mask_edge=args.mask_edge, k=args.kk, step=args.step))
    print(len(dataset))
    loader = DataLoaderContrast5(dataset, batch_size=args.batch_size, shuffle=True, num_workers=args.num_workers)
