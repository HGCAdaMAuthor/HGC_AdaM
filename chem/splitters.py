...
def scaffold_split(dataset, smiles_list, task_idx=None, null_value=0,
    ...
    processed_path = "/home/lxyww7/pretrain-gnns-master/chem/dataset/tox21/processed"

    with open(os.path.join(processed_path, "split_idx_train.pkl"), "wb") as f:
        pickle.dump(torch.tensor(train_idx), f)
        f.close()

    with open(os.path.join(processed_path, "split_idx_valid.pkl"), "wb") as f:
        pickle.dump(torch.tensor(valid_idx), f)
        f.close()

    with open(os.path.join(processed_path, "split_idx_test.pkl"), "wb") as f:
        pickle.dump(torch.tensor(test_idx), f)
        f.close()
