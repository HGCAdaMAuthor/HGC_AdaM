...
from util import MaskAtom, MaskAtomKeptOrigin
...
def train(args, model_list, loader, optimizer_list, device):
    ...
    for step, batch in enumerate(tqdm(loader, desc="Iteration")):
        ...
        node_rep_masked = model(batch.masked_x, batch.edge_index, batch.edge_attr)

        rep_k_global = pool_func(node_rep, batch.batch, "sum")
        rep_q_global = pool_func(node_rep_masked, batch.batch, "sum")
        ...
        # mask
        print(rep_q_global.size())
        mask = torch.zeros((bs, bs)).cuda(device)

        mask[torch.arange(bs, device=device), torch.arange(bs, device=device)] = LARGE_NUM
        ...
        loss_gra = (loss_k + loss_q)

        # print(loss_gra)

        ## loss for nodes
        pred_node = linear_pred_atoms(node_rep_masked[batch.masked_atom_indices])
        loss = criterion(pred_node.double(), batch.mask_node_label[:, 0]) + loss_gra
...
def main():
    ...
    parser.add_argument('--introduce_random', action="store_true", default=False,
                        help='number of workers for dataset loading')
    parser.add_argument('--mask_stra', type=str, default="random", help='number of workers for dataset loading')
    args = parser.parse_args()

    args.output_model_file = "temp/contrast_masking_try"
    torch.manual_seed(0)
    np.random.seed(0)
    ...
    dataset = MoleculeDataset("dataset/" + args.dataset, dataset=args.dataset, transform=MaskAtomKeptOrigin(num_atom_type=119,
                                                                                                  num_edge_type=5,
                                                                                                  mask_rate=args.mask_rate,
                                                                                                  mask_edge=args.mask_edge,
                                                                                                  introduce_random=args.introduce_random,
                                                                                                  mask_stra=args.mask_stra))
