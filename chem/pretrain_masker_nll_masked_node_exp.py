...
def train(args, model_list, loader, optimizer_list, device):
    ...
    for step, batch in enumerate(tqdm(loader, desc="Iteration")):
        ...
        if args.epoch == 1 or args.epoch < 5:
            # if step % 100 == 0:
            masked_nodes_exp_dir = "/apdcephfs/private_meowliu/ft_local/gnn_pretraining/mask_info_exp"
            wf = open(masked_nodes_exp_dir + "/all_gra_dis_epoch_{:d}_batch_{:d}_{:d}".format(args.epoch,
                                                                                              step,
                                                                                              args.mask_times), "wb")
            pickle.dump({"edge_index": batch.edge_index.detach().cpu(),
                         "masked_nodes": masked_nodes_idx.detach().cpu(),
                         "batch": batch.batch.detach().cpu()}, wf)
...
def main():
    ...
    project_dir = "/apdcephfs/private_meowliu/ft_local/gnn_pretraining"
    args.output_model_file = project_dir + \
                             "/temp_model/masker_entropy_based_rever_exp_gnn_{}_mask_times_v2_{:d}_batch_size_{:d}_reverse_{}".format(args.gnn_type, args.mask_times, args.batch_size, str(args.reverse))
    torch.manual_seed(0)
    np.random.seed(0)
    ...
    dataset = MoleculeDataset("/apdcephfs/private_meowliu/ft_local/gnn_pretraining/data/" + args.dataset,
