...
from util import ExtractSubstructureContextPair, PairwiseNodeEdgeMask, PairwiseNodeGCC

from dataloader import DataLoaderSubstructContext, DataLoaderMasking, DataLoaderContrastSubgraph
...
def train(args, model_k, model_q, model_dis, model_attn, loader, optimizer_q, optimizer_dis, optimizer_attn, device
    ...
    for step, batch in enumerate(loader):
        ...
        # print(batch.x_a.size(0))
        rep_q = model_q(batch.x_a.cuda(), batch.edge_index_a.cuda(), batch.edge_attr_a.cuda())
        # rep_q = model_attn(rep_q, batch.mask)
        rep_q_global = pool_func(rep_q, batch.batch_a.cuda(), args.pooling)
        rep_k = model_q(batch.x_b.cuda(), batch.edge_index_b.cuda(), batch.edge_attr_b.cuda())
        # rep_k = model_attn(rep_k, batch.mask)
        rep_k_global = pool_func(rep_k, batch.batch_b.cuda(), args.pooling)
        ...
        # positive_expanded_summary_emb_q = summary_emb_q[batch.batch_a]
        # positive_expanded_summary_emb_k = summary_emb_k[batch.batch_b]
        #
        ...
        # negative_expanded_summary_emb_k = shifted_summary_emb_k[batch.batch_a]
        # negative_expanded_summary_emb_q = shifted_summary_emb_q[batch.batch_b]
        #
        ...
        target = torch.arange(bs).cuda() + bs
...
def main():
    ...
    parser.add_argument("--rsprob", type=float, default=0.8, help="clip norm")
    parser.add_argument("--step_dist", type=list, default=[1.0, 0.0, 0.0], help="clip norm")
    ...
    args.output_model_file = "/apdcephfs/private_meowliu/ft_local/gnn_pretraining/temp_model/contrast_subgraph"

    # set up dataset and transform function.
    dataset = ContrastDataset("/apdcephfs/private_meowliu/ft_local/gnn_pretraining/data/" + args.dataset, args=args, dataset=args.dataset,
                              transform=PairwiseNodeGCC(num_atom_type = 119,
                                                        num_edge_type = 5,
    ...
    loader = DataLoaderContrastSubgraph(dataset, batch_size=args.batch_size, shuffle=True, num_workers=args.num_workers)
    ...
    model_q = GNN(args.num_layer, args.emb_dim, JK=args.JK, drop_ratio=args.dropout_ratio,
                        gnn_type=args.gnn_type).cuda()
    model_dis = Discriminator(args.emb_dim if args.JK != "concat" else args.emb_dim * (1 + 1)).cuda()
    model_attn = atten_module(input_dim=args.emb_dim).cuda()
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        train_loss, train_acc = train(args, model_k, model_q, model_dis, model_attn, loader, optimizer_q, optimizer_dis,
                                      optimizer_attn, device, criterion, epoch)
