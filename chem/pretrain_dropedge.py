...
from util import Extract_Substruct_Contrast, DropEdgePair

from dataloader import DataLoaderContrastPair, DataLoaderMasking
...
def train(args, model, model_dis, model_pred, loader, optimizer, optimizer_dis, optimizer_pred, device, criterion, epoch):
    ...
    # todo add infomax
    # todo make the pretext task to judge which graph is modified?
    # todo: i think that the model based drop strategies are not so possible ---- except for rl based model, which i
    #  have not thought thothroughly...
    #  then can we use the statistic information in the dropping procedure, like calculate the clustering coefficient
    #  and also the hierarchical edge choosing method --- chose the nodes with higher clustering coefficient and also
    #  the other point the higher coefficient clustering ---- but it will introduce the deterministics into the dropping
    #  procedure (the edges dropped are the same at each epochs); so, let's pass the edge dropping method now...
    for step, batch in enumerate(tqdm(loader, desc="Iteration")):
        ...
        rep_droped = model(batch.x, batch.edge_droped_index, batch.edge_droped_attr)

        rep_a_global = pool_func(rep, batch.batch, "sum")
        rep_b_global = pool_func(rep_droped, batch.batch, "sum")
        ...
        if args.use_infomax:
            pos_expanded_embedding = rep_a_global[batch.batch]
            shift_embedding = rep_a_global[cycle_index(rep_a_global.size(0), 1)]
            neg_expanded_embedding = shift_embedding[batch.batch]
            pos_score = model_dis(rep, pos_expanded_embedding)
            neg_score = model_dis(rep, neg_expanded_embedding)
            loss = loss + F.binary_cross_entropy_with_logits(pos_score, torch.ones_like(pos_score)) + \
...
def main():
    ...
    args.output_model_file = "temp/contrast_droped_edge_pair_infomax_{}".format(args.use_infomax)
    l1 = args.num_layer - 1
    l2 = l1 + args.csize
    ...
    dataset = MoleculeDataset("dataset/" + args.dataset, dataset=args.dataset,
                              transform=DropEdgePair(num_atom_type = 119, num_edge_type = 5,  mask_edge=args.mask_edge))
    ...
    model_pred = torch.nn.Linear(args.emb_dim, 119).to(device)
