...
def train(args, model_q, loader, optimizer_q, device, epoch):
    ...
    for step, batch in enumerate(loader):
        if step * args.batch_size >= 500000:
            break
        # print(step)
...
def main():
    ...
    # args.num_samples = 1
    #
    # args.output_model_file = "temp/contrast_sim_based_multi_pos_v3_sample_stra_{}_{:d}_num_neg_samples_{:d}_T_{:d}_sel_other_stra_{}_hop_{:d}_rstprob_{}_p_{:.1f}_q_{:.1f}_rw_hops_{:d}_num_path_{:d}_dl_other_p_w2v_neg_p_no_automl_gnn_{}_sim_exp".format(
    ...
    args.output_model_file = "temp/contrast_sim_based_multi_pos_rw_hops_{:d}_num_path_{:d}_gnn_{}_sim_exp_v2".format(
        args.rw_hops,
        args.num_path,
        args.gnn_type)
    ...
    dataset = MoleculeDatasetForContrast(dataset_root_path, dataset=args.dataset, transform=None,
                                         ...
                                         args=args)
    loader = DataLoaderContrastSimBasedMultiPosComNeg(dataset, batch_size=args.batch_size, shuffle=True,
