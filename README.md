...
- Social network pre-training datasets `Soc_S_NF` and `Soc_L_NF` are collected from social classification datasets from [TUDataset](https://chrsmrrs.github.io/datasets/docs/datasets/). Some pre-processing is needed in order to get data that can be used in the pre-training stage directly. We upload the pre-processed data, which can be downloaded as stated in the following. 
- For molecular graph pre-training dataset `MolD`, please refer to [snap-pretrain](https://github.com/snap-stanford/pretrain-gnns) and proceed following their description for `chem data`. Some pre-processing is needed in order to get data that can be used in the pre-training stage directly. We also upload the pre-processed data, which can be downloaded as stated in the following. 
